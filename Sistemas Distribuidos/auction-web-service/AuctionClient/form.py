    ...
    def update_list(self):
        try:
            ...
            for item in self.items:
                ...
                if item["owner"] == self.client.id:
                    widget.setForeground(QColor("#e5c07b"))
                    widget.setCheckState(Qt.CheckState.Checked)
                elif self.client.id in item["biders"]:
...
class Login(QMainWindow):
    ...
    def login_clicked(self):
        ...
        if name and host:
            try:
                if self.client.login(name, host):
                    self.hide()
                    self.parent.show()
            ...
            QMessageBox.critical(self, "Error!", "You must provide your name and the web service URL to login.")
